...
class NagiosImportShell extends AppShell {
    ...
    public function parseDir($path, $override, $clear) {
        $all_files = scandir($path);
        //This two could done parallel
        if (in_array("commands", $all_files)) {
            if ($clear) {
                $this->clearDatabase(new Command(), "commands");
            }
            $file = $all_files[array_search("commands", $all_files)];
            $this->scanConfigFilesInDir($path, $file, $override, $clear);
        }

        if (in_array("timeperiods", $all_files)) {
            if ($clear) {
                $this->clearDatabase(new Timeperiod(), "timeperiods");
                $this->clearDatabase(new Timerange(),"timeperiod_timeranges");
            }
            $file = $all_files[array_search("timeperiods", $all_files)];
            $this->scanConfigFilesInDir($path, $file, $override, $clear);
        }

        //This must be done separat
        if (in_array("contacts", $all_files)) {
            if ($clear) {
                $this->clearDatabase(new Contact(), "contacts");
            }
            $file = $all_files[array_search("contacts", $all_files)];
            $this->scanConfigFilesInDir($path, $file, $override, $clear);

        }

        //This must be done separate
        if (in_array("contactgroups", $all_files)) {
            if ($clear) {
                $this->clearDatabase(new Contactgroup(), "contactgroups");
            }
            $file = $all_files[array_search("contactgroups", $all_files)];
            $this->scanConfigFilesInDir($path, $file, $override, $clear);
        }

        //This two could done parallel
        if (in_array("hosttemplates", $all_files)) {
            if ($clear) {
                $this->clearDatabase(new Hosttemplate(), "hosttemplates");
            }
            $file = $all_files[array_search("hosttemplates", $all_files)];
            $this->scanConfigFilesInDir($path, $file, $override, $clear);
        }

        if (in_array("servicetemplates", $all_files)) {
            if ($clear) {
                $this->clearDatabase(new Servicetemplate(), "servicetemplates");
            }
            $file = $all_files[array_search("servicetemplates", $all_files)];
            $this->scanConfigFilesInDir($path, $file, $override, $clear);
        }

        //This must be done separat
        if (in_array("hosts", $all_files)) {
            if ($clear) {
                $this->clearDatabase(new Host(), "hosts");
            }
            $file = $all_files[array_search("hosts", $all_files)];
            $this->scanConfigFilesInDir($path, $file, $override, $clear);
        }

        //This must be done separat
        if (in_array("services", $all_files)) {
            if ($clear) {
                $this->clearDatabase(new Service(), "services");
            }
            $file = $all_files[array_search("services", $all_files)];
            $this->scanConfigFilesInDir($path, $file, $override, $clear);
        }

        //This two could done parallel
        if (in_array("hostgroups", $all_files)) {
            if ($clear) {
                $this->clearDatabase(new Hostgroup(), "hostgroups");
            }
            $file = $all_files[array_search("hostgroups", $all_files)];
            $this->scanConfigFilesInDir($path, $file, $override, $clear);
        }

        if (in_array("servicegroups", $all_files)) {
            if ($clear) {
                $this->clearDatabase(new Servicegroup(), "servicegroups");
            }
            $file = $all_files[array_search("servicegroups", $all_files)];
            $this->scanConfigFilesInDir($path, $file, $override, $clear);
        }

        //This four could done parallel
        if (in_array("hostdependencies", $all_files)) {
            if ($clear) {
                $this->clearDatabase(new Hostdependency(), "hostdependencies");
            }
            $file = $all_files[array_search("hostdependencies", $all_files)];
            $this->scanConfigFilesInDir($path, $file, $override, $clear);
        }

        if (in_array("servicedependencies", $all_files)) {
            if ($clear) {
                $this->clearDatabase(new Servicedependency(), "servicedependencies");
            }
            $file = $all_files[array_search("servicedependencies", $all_files)];
            $this->scanConfigFilesInDir($path, $file, $override, $clear);
        }

        if (in_array("hostescalations", $all_files)) {
            if ($clear) {
                $this->clearDatabase(new Hostescalation(), "hostescalations");
            }
            $file = $all_files[array_search("hostescalations", $all_files)];
            $this->scanConfigFilesInDir($path, $file, $override, $clear);
        }

        if (in_array("serviceescalations", $all_files)) {
            if ($clear) {
                $this->clearDatabase(new Serviceescalation(), "serviceescalations");
            }
            $file = $all_files[array_search("serviceescalations", $all_files)];
            $this->scanConfigFilesInDir($path, $file, $override, $clear);
        }
    }


    private function scanConfigFilesInDir($path, $file, $override, $clear){
        if (is_dir($path."/".$file) && $file != "." && $file != "..") {
            $config_files = scandir($path."/".$file);
            $this->out("<info>Parsing configurations for ".$file."</info>");
            ...
            foreach ($config_files as $config_file) {
                if (!is_dir($config_file)) {
                    $this->parseFile($path."/".$file."/".$config_file, $override, $clear);
                }
            }
        }
    }
    private function clearDatabase($object, $table) {
        //Truncate Table
        $object->query('TRUNCATE TABLE '.$table.';');
    }
    private function saveData($object, $array) {
        //Save all Data which has been parsed
        if(!$object->saveAll($array)) {
            $this->log($array);
            $this->log($object->validationErrors);
        }
        /*foreach ($array as $conf) {
            if(!$object->saveAll($conf)) {
                $this->log($conf);
                $this->log($object->validationErrors);
            }
        }*/
    }
    public function parseFile($file, $override, $clear) {
        ...
        if (strstr($file,$this->conf["suffix"])) {
            ...
            $cfg_file = file($file, FILE_SKIP_EMPTY_LINES);
            switch($config_type){
                case "commands":
                    //Read the config file and check if it was empty
                    if (!$this->readCommandConfigFile($cfg_file, false, $override, $clear)) {
                        $this->out("<warning>No Command defined</warning>");
                    }
                    break;
                case "contactgroups":
                    //Read the config file and check if it was empty
                    if (!$this->readContactgroupConfigFile($cfg_file, false, $override, $clear)) {
                        $this->out("<warning>No Contactgroup defined</warning>");
                    }
                    break;
                case "contacts":
                    //Read the config file and check if it was empty
                    if (!$this->readContactsConfigFile($cfg_file, false, $override, $clear)) {
                        $this->out("<warning>No Contact defined</warning>");
                    }
                    break;
                case "hostdependencies":
                    //Read the config file and check if it was empty
                    if (!$this->readHostdependencyConfigFile($cfg_file, false, $override, $clear)) {
                        $this->out("<warning>No Hostdependencie defined</warning>");
                    }
                    break;
                case "hostescalations":
                    //Read the config file and check if it was empty
                    if (!$this->readHostescalationConfigFile($cfg_file, false, $override, $config_name[0], $clear)) {
                        $this->out("<warning>No Hostescalation defined</warning>");
                    }
                    break;
                case "hostgroups":
                    //Read the config file and check if it was empty
                    if (!$this->readHostgroupConfigFile($cfg_file, false, $override, $clear)) {
                        $this->out("<warning>No Hostgroup defined</warning>");
                    }
                    break;
                case "hosts":
                    //Read the config file and check if it was empty
                    if (!$this->readHostConfigFile($cfg_file, false, $override, $clear)) {
                        $this->out("<warning>No Host defined</warning>");
                    }
                    break;
                case "hosttemplates":
                    //Read the config file and check if it was empty
                    if (!$this->readHosttemplateConfigFile($cfg_file, false, $override, $clear)) {
                        $this->out("<warning>No Hosttemplate defined</warning>");
                    }
                    break;
                case "servicedependencies":
                    //Read the config file and check if it was empty
                    if (!$this->readServicedependencyConfigFile($cfg_file, false, $override, $config_name[0], $clear)) {
                        $this->out("<warning>No Service-Dependencies defined</warning>");
                    }
                    break;
                case "serviceescalations":
                    //Read the config file and check if it was empty
                    if (!$this->readServiceescalationConfigFile($cfg_file, false, $override, $config_name[0], $clear)) {
                        $this->out("<warning>No Service-Escalation defined</warning>");
                    }
                    break;
                case "servicegroups":
                    //Read the config file and check if it was empty
                    if (!$this->readServicegroupConfigFile($cfg_file, false, $override, $config_name[0], $clear)) {
                        $this->out("<warning>No Service-Group defined</warning>");
                    }
                    break;
                case "services":
                    //Read the config file and check if it was empty
                    if (!$this->readServiceConfigFile($cfg_file, false, $override, $clear)) {
                        $this->out("<warning>No Service defined</warning>");
                    }
                    break;
                case "servicetemplates":
                    //Read the config file and check if it was empty
                    if (!$this->readServicetemplateConfigFile($cfg_file, false, $override, $clear)) {
                        $this->out("<warning>No Service-Template defined</warning>");
                    }
                    break;
                case "timeperiods":
                    //Read the config file and check if it was empty
                    if (!$this->readTimeperiodConfigFile($cfg_file, false, $override, $clear)) {
                        $this->out("<warning>No Service-Group defined</warning>");
                    }
                    break;
                case "defaults":
    ...
    private function readCommandConfigFile($cfg_file, $newObj = false, $override, $clear){
        $saveDataArray = null;
        $cnt = 0;
            ...
            switch ($line_parts[0]) {
                ...
                case "}":
                    if (($override) || (!$override && $this->mainObj->uuid != null)) {
                        $this->saveData($this->mainObj, $saveDataArray);
        ...
        //$this->saveData($this->mainObj, $this->commands_configutation);
        return $newObj;
    }

    private function readContactgroupConfigFile($cfg_file, $newObj = false, $override, $clear) {
        $contacts = null;
        $saveDataArray = null;
        $container = null;
        $cnt = 0;
        ...
        //$this->saveData($this->mainObj, $this->contactgroups_configutation);
        return $newObj;
    }

    private function readContactsConfigFile($cfg_file, $newObj = false, $override, $clear){
        $host_commands = null;
            ...
            switch ($line_parts[0]) {
                ...
                case "email":
                    if (sizeof($line_parts) > 1){
                        ...
                        $saveDataArray['Contact']['email'] =  "";
                    }
                    break;
                case "pager":
                    $saveDataArray['Contact']['phone'] =  trim($line_parts[1]);
                    break;
                case "}":
                    if (($override) || (!$override && $this->mainObj->uuid != null)) {
                        $this->saveData($this->mainObj, $saveDataArray);
        ...
        //$this->saveData($this->mainObj, $this->contacts_configutation);
        return $newObj;
    }

    private function readHostdependencyConfigFile($cfg_file, $newObj = false, $override, $clear){
        ...
        //$this->saveData($this->mainObj, $this->hostdependencies_configutation);
        return $newObj;
    }

    private function readHostescalationConfigFile($cfg_file, $newObj = false, $override, $name, $clear){
        $contacts = null;
        $contact_groups = null;
        $hosts = null;
        $host_groups = null;
        ...
        //$this->saveData($this->mainObj, $this->hostescalations_configutation);
        return $newObj;
    }

    private function readHostgroupConfigFile($cfg_file, $newObj = false, $override, $clear){
        $hosts = null;
        $hostgroup = null;
        $saveDataArray = null;
        ...
        //$this->saveData($this->mainObj, $this->hostgroups_configutation);
        return $newObj;
    }

    private function readHostConfigFile ($cfg_file, $newObj = false, $override, $clear) {
        $contacts = null;
        $contact_groups = null;
        $check_cmds = null;
        $saveDataArray = null;
        ...
        //$this->saveData($this->mainObj, $this->hosts_configutation);
        return $newObj;
    }

    private function readHosttemplateConfigFile($cfg_file, $newObj = false, $override, $clear){
        $contacts = null;
        $contact_groups = null;
        $check_cmds = null;
        $saveDataArray = null;
        ...
        for($i=0; $i < count($cfg_file); $i++) {
            ...
            switch ($line_parts[0]) {
                ...
                case "}":
                    ...
                    $saveDataArray['Contact'] = $contacts;
                    $saveDataArray['Hosttemplate']['Contactgroup'] = $contact_groups;
                    $saveDataArray['Contactgroup'] = $contact_groups;
                    if (($override) || (!$override && $this->mainObj->uuid != null)) {
                        $this->saveData($this->mainObj, $saveDataArray);
        ...
        //$this->saveData($this->mainObj, $this->hosttemplates_configutation);
        return $newObj;
    }

    private function readServicedependencyConfigFile($cfg_file, $newObj = false, $override, $name, $clear){
        $service = null;
        ...
        //$this->saveData($this->mainObj, $this->servicedependencies_configutation);
        return $newObj;
    }

    private function readServiceescalationConfigFile($cfg_file, $newObj = false, $override, $name, $clear){
        $contacts = null;
        $contact_groups = null;
        $saveDataArray = null;
        ...
        //$this->saveData($this->mainObj, $this->servicescalations_configutation);
        return $newObj;
    }

    private function readServicegroupConfigFile($cfg_file, $newObj = false, $override, $name, $clear) {
        $services = null;
        $saveDataArray = null;
        $cnt = 0;
        ...
        //$this->saveData($this->mainObj, $this->servicegroups_configutation);
        return $newObj;
    }

    private function readServiceConfigFile($cfg_file, $newObj = false, $override, $clear) {
        $contacts = null;
        $contact_groups = null;
        $saveDataArray = null;
        $service_groups = null;
        ...
        for($i=0; $i < count($cfg_file); $i++) {
            ...
            switch ($line_parts[0]){
                ...
                case "}":
                    ...
                    $saveDataArray['Contact']['Contact'] = $contacts;
                    $saveDataArray['Service']['Contactgroup'] = $contact_groups;
                    $saveDataArray['Contactgroup']['Contactgroup'] = $contact_groups;
        ...
        //$this->saveData($this->mainObj, $this->services_configutation);
        return $newObj;
    }

    private function readServicetemplateConfigFile($cfg_file, $newObj = false, $override, $clear) {
        $contacts = null;
        $contact_groups = null;
        $saveDataArray = null;
        $check_cmds = null;
            ...
            switch ($line_parts[0]){
                ...
                case "name":
                    ...
                    if ($id != null && !$clear) {
                        ...
                        $saveDataArray['Servicetemplate']['notify_period_id'] = "1";
                    } else {
                        $this->mainObj->uuid = $this->checkUUID($line_parts[1]);
                        $this->mainObj->id = null;
                        $saveDataArray['Servicetemplate']['uuid'] = $this->checkUUID($line_parts[1]);
                        $saveDataArray['Servicetemplate']['name'] = $this->checkUUID($line_parts[1]);
                ...
                case "check_command":
                    ...
                    if ($id != null) {
                        ...
                        $this->out($uuid. " = leere command id");
                        $saveDataArray['Servicetemplate']['command_id'] = "";
                    }
        ...
        //$this->saveData($this->mainObj, $this->servicetemplates_configutation);
        return $newObj;
    }

    private function readTimeperiodConfigFile($cfg_file, $newObj = false, $override, $clear) {
        $saveDataArray = null;
        $timerangeArr = null;
        $id = null;
        $cnt = 0;
            ...
            switch ($line_parts[0]){
                ...
                case "timeperiod_name":
                    ...
                    if ($id != null && !$clear) {
                        ...
                        $saveDataArray['Timeperiod']['name'] = $this->checkUUID($line_parts[1]);
        ...
        //$this->saveData($this->mainObj, $this->timeperiods_configutation);
        return $newObj;
    }
    ...
    private function readCheckCommand($line, $objekt_cmd_args, $command, $type, &$array){
        ...
        $params = explode(";", $line);
        $command_params = explode("!", trim($params[0]));
